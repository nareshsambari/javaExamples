...
public class PlayersDao {
	...
	public void getPlayerByIdWithMapper(int id) {
		...
		PlayersModel model = jt.queryForObject(sql, new Object[] { id }, new PlayersRowMapper());
		System.out.println(model);
	}

	public void getPlayersWithRowMapper() {
		Map<Integer, PlayersModel> pMap = new HashMap();
		String sql = "SELECT * FROM CRICKET_PLAYERS";
		List<PlayersModel> modelsList = jt.query(sql, new PlayersRowMapper());
		Iterator<PlayersModel> itr = modelsList.iterator();
		while (itr.hasNext()) {
			PlayersModel pModel = itr.next();
	...
	public void insertUsingBatch(List<PlayersModel> pList) {
		String sql = "insert into cricket_players values(?,?,?)";
		List<Object[]> paramsArr = new ArrayList<Object[]>();
		for (PlayersModel model : pList) {
